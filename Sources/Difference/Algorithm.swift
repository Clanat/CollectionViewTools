...
public extension StagedChangeset
    where Collection: RangeReplaceableCollection, Collection.Element: CollectionViewCellItem {
...
extension StagedChangeset
    where Collection: RangeReplaceableCollection,
          Collection.Element: DifferentiableCollectionViewSectionItem {
    ...
    @inlinable
    init(source: Collection, target: Collection) {
        typealias SectionItem = Collection.Element
        typealias SectionItemIdentifier = SectionItem.DifferenceIdentifier
        typealias CellItem = SectionItem.CellItem
        typealias CellItemElementIdentifier = CellItem.DifferenceIdentifier

        let sourceSections = ContiguousArray(source)
        let targetSections = ContiguousArray(target)

        let contiguousSourceSections = ContiguousArray(sourceSections.map { ContiguousArray($0.cellItems) })
        let contiguousTargetSections = ContiguousArray(targetSections.map { ContiguousArray($0.cellItems) })

        var firstStageSections = sourceSections
        var secondStageSections = ContiguousArray<SectionItem>()
        var thirdStageSections = ContiguousArray<SectionItem>()
        var fourthStageSections = ContiguousArray<SectionItem>()
        ...
        var flattenSourceIdentifiers = ContiguousArray<CellItemElementIdentifier>()
        ...
        flattenSourceIdentifiers.withUnsafeBufferPointer { bufferPointer in
            ...
            var sourceOccurrencesTable = [TableKey<CellItemElementIdentifier>: Occurrence](minimumCapacity: flattenSourceCount * 2)
        ...
        for sourceSectionIndex in contiguousSourceSections.indices {
            ...
            if case .some = sectionResult.metadata.sourceTraces[sourceSectionIndex].reference {
                ...
                var secondStageElements = ContiguousArray<CellItem>()
                ...
                let secondStageSection = SectionItem(sectionItem: sourceSection)
                // FIXME: avoid performance loss (casting to array)
                secondStageSection.cellItems = Array(secondStageElements)
                secondStageSections.append(secondStageSection)
            }

            let firstStageSection = SectionItem(sectionItem: sourceSection)
            // FIXME: avoid performance loss (casting to array)
            firstStageSection.cellItems = Array(firstStageElements)
        ...
        for targetSectionIndex in contiguousTargetSections.indices {
            ...
            var fourthStageElements = ContiguousArray<CellItem>()
            ...
            let fourthStageSection = SectionItem(sectionItem: thirdStageSection)
            // FIXME: avoid performance loss (casting to array)
            fourthStageSection.cellItems = Array(fourthStageElements)
